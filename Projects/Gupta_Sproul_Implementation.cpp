...
void plot(float x, float y, float brightness) {
    // Scale and center the coordinates to fit OpenGL's normalized space (-1 to 1)
...
void GuptaSproullLine(float x1, float y1, float x2, float y2) {
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(500, 500, "Gupta-Sproull Algorithm", NULL, NULL);
    if (!window) { glfwTerminate(); return -1; }
    glfwMakeContextCurrent(window);
    glewInit();

    GuptaSproullLine(-2, 3, 1, 4); // Draw the line
